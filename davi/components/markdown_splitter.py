...
class CustomDocumentSplitter(DocumentSplitter):
    ...
    def __init__(self, *args, page_break_character="\\f", **kwargs):
        ...
        def default_splitter(content: str) -> List[Dict[Any, Any]]:
            return [{"content": content, "meta": {}}]

        self.splitting_function = default_splitter

    def _flatten_dict(
        self, d: Dict, prefix: str = "", target_dict: Optional[Dict] = None
    ) -> Dict:
    ...
    def _split_by_function(self, doc: Document) -> List[Document]:
        ...
        if doc.content is None:
            logger.debug(f"Empty content in document with id={doc.id}")
            return [doc]

        if self.splitting_function is None:
            return [doc]
        splits: List[dict] = self.splitting_function(doc.content)  # type: ignore
        docs = []

        # calculate total pages and set current page
        content_page_count = (
            1 if doc.content is None else doc.content.count(self.page_break_character)
        )
        total_pages = doc.meta.get("total_pages", 0) or content_page_count
        current_page = doc.meta.get("page_number", 1)
        logger.debug(
            f"Starting page number: {current_page}, Total pages: {total_pages}"
        )
        ...
        for i, split in enumerate(splits):
            ...
            if split.get("meta"):
                meta.update(self._flatten_dict(split.get("meta")))  # type: ignore

            docs.append(Document(content=split["content"], meta=meta))

        logger.debug(
            f"Split into {len(docs)} documents for id={doc.id}, final page: {current_page}"
        )
        return docs
...
class MarkdownHeaderSplitter:
    ...
    def _split_by_markdown_headers(
        self, text: str, enforce_first_header: Optional[bool] = None
    ) -> List[Dict]:
        ...
        if not matches:
            logger.info(
                "No headers found in document; returning full document as single chunk."
            )
            return [{"content": text, "meta": {"header": None, "parentheaders": []}}]

        # process headers and build chunks
        chunks = []
        header_stack: List[Any] = [None] * 6
        active_parents = []
        ...
        for i, match in enumerate(matches):
            ...
            # get parent headers
            parentheaders: List[Any] = list(active_parents)

            # enforce first header if needed
            if (
                enforce_first_header
                and first_header
                and (not parentheaders or parentheaders[0] != first_header)
            ):
                parentheaders = [first_header] + [
                    h for h in parentheaders if h != first_header
                ]
    ...
    def _process_split(
        self, secondary_splits, current_page, header_prefix, first_header, doc
    ) -> List[Document]:
        result_docs = []
    ...
    def _apply_secondary_splitting(self, documents: List[Document]) -> List[Document]:
        ...
        for doc in documents:
            ...
            result_docs = self._process_split(
                doc=doc,
        ...
        logger.info(
            f"Secondary splitting complete. Final count: {len(result_docs)} documents."
        )
        return result_docs

    @component.output_types(documents=List[Document])
    def run(
        self, documents: List[Document], enforce_first_header: Optional[bool] = None
        ...
        """
        logger.info(
            f"Processing {len(documents)} documents with enforce_first_header={enforce_first_header}"
        )

        # split by markdown headers
        header_splitter = CustomDocumentSplitter(
            split_by="function",
            splitting_function=lambda text: self._split_by_markdown_headers(
                text, enforce_first_header
            ),
